...
import md5 from '@/sheep/platform/md5.js'
...
uni.getSystemInfo({
	success: (res) => {
		uni.setStorageSync('deviceModel', res.deviceModel)
...
const http = new Request({
	...
	header: {
		...
		'deviceModel': uni.getStorageSync('deviceModel'),
...
http.interceptors.request.use(
	(config) => {
		...
		let sign = md5.md5(md5.sortASCII(config.data));
		if (sign) config.header['sign'] = sign.toUpperCase();
		if (token) config.header['laytp-admin-token'] = token;
		return config;
	},
	(error) => {
